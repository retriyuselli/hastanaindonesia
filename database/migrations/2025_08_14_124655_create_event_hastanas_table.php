...
return new class extends Migration
{
    ...
    {
        Schema::create('event_hastanas', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('status', ['draft', 'published', 'cancelled', 'finished'])->default('draft'); // Status event
            $table->json('schedule')->nullable(); // JSON array of event schedule
            $table->integer('quota')->nullable(); // Kuota peserta
            $table->enum('event_type', ['internal', 'eksternal']); // Jenis kegiatan
    ...
    {
        Schema::dropIfExists('event_hastanas');
    }
};
