...
class EventHastanaSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($allEvents as $eventData) {
            try {
                $quota = (int) ($eventData['quota'] ?? 0);
                $maxParticipants = (int) ($eventData['max_participants'] ?? 0);
                $capacitySource = $maxParticipants > 0 ? $maxParticipants : ($quota > 0 ? $quota : 10);
                $capacity = min($capacitySource, 10);

                $eventData['quota'] = $capacity;
                $eventData['max_participants'] = $capacity;
                $eventData['current_participants'] = min((int) ($eventData['current_participants'] ?? 0), $capacity);
